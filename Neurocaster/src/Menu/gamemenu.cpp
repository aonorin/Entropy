    ...
    visible(true)
{
    ...
    Ogre::ConfigOptionMap& CurrentRendererOptions = Neurocaster::getSingletonPtr()->mRoot->getRenderSystem()->getConfigOptions();
    ...
    //Blackout screen
    Ogre::MaterialPtr blackoutMat = Ogre::MaterialManager::getSingleton().create("blackoutMat", Ogre::ResourceGroupManager::DEFAULT_RESOURCE_GROUP_NAME);
    blackoutMat->getTechnique(0)->getPass(0)->createTextureUnitState("InventoryBlackout.png");
    blackoutMat->getTechnique(0)->getPass(0)->setDepthCheckEnabled(false);
    blackoutMat->getTechnique(0)->getPass(0)->setDepthWriteEnabled(false);
    blackoutMat->getTechnique(0)->getPass(0)->setLightingEnabled(false);
    blackoutMat->getTechnique(0)->getPass(0)->setSceneBlending(Ogre::SBF_SOURCE_ALPHA, Ogre::SBF_ONE_MINUS_SOURCE_ALPHA);

    blackoutOverlay = overlayMgr->create("blackoutOverlay");
    blackoutOverlay->setZOrder(10);
    blackoutOverlay->setScale(1,1);
    blackoutOverlay->hide();

    blackout = static_cast<Ogre::OverlayContainer*>(overlayMgr->createOverlayElement("Panel", "blackout"));
    blackout->setMetricsMode(Ogre::GMM_RELATIVE);
    blackout->setHorizontalAlignment(Ogre::GHA_LEFT);
    blackout->setVerticalAlignment(Ogre::GVA_TOP);
    blackout->setDimensions(1.0,1.0);
    blackout->setPosition(0,0);
    blackout->show();
    blackout->setMaterialName("blackoutMat");
    blackoutOverlay->add2D(blackout);
...
void GameMenu::getCurrentSettings()
{
    ...
    std::string resString =
            boost::lexical_cast<std::string>(Neurocaster::getSingletonPtr()->mWindow->getViewport(0)->getActualWidth()) +
            " x " +
            boost::lexical_cast<std::string>(Neurocaster::getSingletonPtr()->mWindow->getViewport(0)->getActualHeight());
    ...
    //Get Current FSAA
    fsaa = Neurocaster::getSingletonPtr()->mWindow->getFSAA();
    fullscreen = Neurocaster::getSingletonPtr()->mWindow->isFullScreen();
}
...
void GameMenu::setupMainMenu()
{
    ...
    addButton("exitMain","mainMenu",3,"Resume");
...
bool GameMenu::insideElement(std::string elementName)
{
    ...
    if(
            ...
            mouseX <= element->getLeft()+element->getWidth() &&
            mouseY >= element->getTop() &&
            mouseY <= element->getTop()+element->getHeight()
            ) {
        return true;
    } else {
        return false;
...
{
    Neurocaster::getSingletonPtr()->mRoot->getRenderSystem()->setConfigOption("Video Mode",foundResolutions.at(resolution));
    Neurocaster::getSingletonPtr()->mRoot->getRenderSystem()->setConfigOption("FSAA",boost::lexical_cast<std::string>(fsaa));
    if(fullscreen) {
        Neurocaster::getSingletonPtr()->mRoot->getRenderSystem()->setConfigOption("Full Screen","Yes");
    } else  {
        Neurocaster::getSingletonPtr()->mRoot->getRenderSystem()->setConfigOption("Full Screen","No");
    }
    ...
    config << "Render System="<< Neurocaster::getSingletonPtr()->mRoot->getRenderSystem()->getName()<<std::endl;
    config << std::endl;
    config << "["<<Neurocaster::getSingletonPtr()->mRoot->getRenderSystem()->getName()<<"]"<<std::endl;
...
#endif
//    Neurocaster::getSingletonPtr()->mWindow->setFullscreen(fullscreen,width,height);
//    config <<
//    Neurocaster::getSingletonPtr()->mWindow->getViewport(0)->setDimensions(0,0,width,height);
    /*
    Neurocaster::getSingletonPtr()->mWindow->setFullscreen(
                fullscreen,
                width,
                height
                );
    Neurocaster::getSingletonPtr()->mWindow->getViewport(0)->update();
    */
//    Neurocaster::getSingletonPtr()->mRoot->getRenderSystem()->reinitialise();
//    Neurocaster::getSingletonPtr()->mRoot->getRenderSystem()->setConfigOption();
    //    Neurocaster::getSingletonPtr()->mWindow->
}
...
{
    Neurocaster::getSingletonPtr()->shutdownGame();
}
...
void GameMenu::startNewGame()
{
    ...
    mouseContainer->hide();
    visible = false;
    Neurocaster::getSingletonPtr()->startNewGame();
}
...
void GameMenu::continueGameM()
{
    ...
    mouseContainer->hide();
    visible = false;
    Neurocaster::getSingletonPtr()->continueGame();
}
