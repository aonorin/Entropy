...
unsigned int AbstractGrid::at(int index, ...)
{
    va_list vl;
    va_start(vl, index);
    unsigned int* indexes = new unsigned int[dimensions];
    indexes[0] = index;
...
void AbstractGrid::randomize()
{
    ...
    for(int i = 0; i < length; ++i)
    {
        unsigned int random = rand() % possibleStates;
        gridArray[i] = random;
        futureGridArray[i] = random;
    }
...
const std::vector<unsigned int> &AbstractGrid::getExtents()
{
    return extents;
}
...
void AbstractStream::calculateRuleArraySize()
{
    ...
    unsigned int* stateArray = new unsigned int[neighbors];
...
void AbstractStream::createRuleArray()
{
    ...
    else
    {
        ...
        std::tr1::unordered_map<double, unsigned int>::iterator iter = ruleMap.begin();
