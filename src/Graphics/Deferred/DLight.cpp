...
void DLight::updateFromCamera(Ogre::Camera* camera)
{
	...
	for (unsigned short i=0; i<tech->getNumPasses(); i++) 
	{
		...
		{
//            params->setNamedConstant("lightPositionObj",mParentLight->getDerivedPosition());
//            fParams->setNamedConstant("lightPositionObj",mParentLight->getDerivedPosition());

            if (params->_findNamedConstantDefinition("playerCameraPosition"))
            {
                params->setNamedConstant("playerCameraPosition",camera->getDerivedPosition());
            }
            if (params->_findNamedConstantDefinition("lightPos"))
            {
                params->setNamedConstant("lightPos",mParentLight->getDerivedPosition());
            }
//            fParams->setNamedAutoConstant("playerCameraPosition",camera->getPosition());
